...
function ProjectSettingBoardInfo({
  ...
  const setProjectSettingBoardInfoProjectId = useSetRecoilState(
    projectSettingBoardInfoSelector('projectId'),
  );
  const setProjectSettingBoardInfoAuthMap = useSetRecoilState(
    projectSettingBoardInfoSelector('authMap'),
  );
  ...
  const { title, boardPositions, contact, recruitmentStatus, content } =
    data!.data!;
