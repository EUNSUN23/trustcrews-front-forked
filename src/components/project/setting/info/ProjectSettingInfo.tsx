...
import useProjectInfoSummary, {
  isQueryDataReady,
} from '@/hooks/common/useProjectInfoSummary';
...
function ProjectSettingInfo({
  ...
  const setProjectSettingProjectId = useSetRecoilState(
    projectSettingInfoSelector('projectId'),
  );
  const setProjectSettingAuthMap = useSetRecoilState(
    projectSettingInfoSelector('authMap'),
  );
  const resetProjectSettingInfoState = useResetRecoilState(
    projectSettingInfoStateStore,
  );

  const { data, isPending, isRefetching, isError, isRefetchError } =
  ...
  useEffect(() => {
    ...
    projectId,
    authMap,
    setProjectSettingProjectId,
    setProjectSettingAuthMap,
    resetProjectSettingInfoState,
  ]);

  const isDataReady = isQueryDataReady(
  ...
  if (!isDataReady) {
    ...
    if (isError || isRefetchError)
      settingBody = <div>데이터 조회에 실패했습니다.</div>;
  } else {
    const {
      projectName,
      projectSubject,
