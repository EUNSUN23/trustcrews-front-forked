...
const TrustGradeSelect = ({
  trustGradeId,
  setTrustGrade,
}: TrustGradeSelectProps) => {
  const { gradeList, isFetching } = useGradeListByUser();

  if (isFetching)
    return (
      <SelectSkeleton label='프로젝트 신뢰등급' placeholder='신뢰등급 선택' />
    );

  const selectItems: SelectItem<Name, Value>[] =
