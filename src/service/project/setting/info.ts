 ...
 * @param reqData
 */
export const updateProjectSettingInfo = async (
  reqData: ProjectSettingInfoUpdReqData,
) => {
  throwErrorIfInvalid(
    isEqual(reqData.projectName, ''),
    '프로젝트 이름을 입력해주세요',
  );
  throwErrorIfInvalid(
    isEqual(reqData.projectSubject, ''),
    '프로젝트 주제를 입력해주세요',
  );
  throwErrorIfInvalid(
    isEqual(reqData.startDate, ''),
    '프로젝트 시작날짜를 입력해주세요',
  );
  throwErrorIfInvalid(
    isEqual(reqData.endDate, ''),
    '프로젝트 종료날짜를 입력해주세요',
  );
  throwErrorIfInvalid(
    isEqual(reqData.technologyIds, []),
    '프로젝트 기술스택을 선택해주세요.',
  );
