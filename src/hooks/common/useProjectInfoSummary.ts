...
import { getProjectSettingInfo } from '@/service/project/setting/info';
import { numStrToBigInt } from '@/utils/common';

export function isQueryDataReady<T>(
...
export default function useProjectInfoSummary(projectId: string) {
  const { data, error, isPending, isRefetching, isError, isRefetchError } =
    useQuery<
      ResponseBody<ProjectInfoSummary>,
      Error,
      ResponseBody<ProjectInfoSummary>
    >({
